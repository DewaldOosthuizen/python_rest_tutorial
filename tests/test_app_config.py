...
class TestMongoURIConfig(unittest.TestCase):
    ...
    def test_mongo_uri_env_var_used_as_default(self):
        ...
        self.assertIn('os.environ.get', source,
                      "app.py must use os.environ.get to read MONGO_URI")
        self.assertIn('MONGO_URI', source,
                      "app.py must reference the MONGO_URI environment variable")
    ...
    def test_mongo_uri_fallback_default(self):
        ...
        with open(web_app_path) as f:
            source = f.read()
        self.assertIn('mongodb://my_db:27017/', source,
                      "Default MONGO_URI fallback must be 'mongodb://my_db:27017/'")
